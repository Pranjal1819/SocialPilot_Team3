...
interface CampaignsState {
  ...
  hasLoaded: boolean;
  error: string | null;
  fetchCampaigns: (businessOwnerId?: string) => void;
  deleteCampaign: (id: string) => Promise<void>;
  addCampaign: (
    campaign: Omit<Campaign, "id" | "scheduledPosts" | "publishedPosts">,
...
export const useCampaignsStore = create<CampaignsState>((set, get) => ({
  ...
  fetchCampaigns: (businessOwnerId?: string) => {
    if (get().isLoading) return;
    set({ isLoading: true, error: null });

    const params = businessOwnerId ? { business_owner_id: businessOwnerId } : undefined;
  ...
  addCampaign: async (input, businessOwnerId) => {
    const { data } = await api.post("/campaigns/", buildCampaignPayload(input), {
      params: businessOwnerId ? { business_owner_id: businessOwnerId } : undefined,
    });
