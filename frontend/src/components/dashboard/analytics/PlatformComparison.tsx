...
const PLATFORM_CHART_COLORS = ["#e85d75", "#2f7dbd", "#e33b32", "#f4c430", "#3f6b4e", "#6f7f91"];
...
export default function PlatformComparison() {
  ...
  const platformShare = rows
    .map((row) => ({ name: PLATFORM_META[row.platform].label, value: row.engagement, platform: row.platform }))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <ChartCard title="Overall Platform Engagement">
          {platformShare.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={platformShare} dataKey="value" nameKey="name" innerRadius={58} outerRadius={88} paddingAngle={3}>
                  {platformShare.map((row, index) => <Cell key={row.platform} fill={PLATFORM_CHART_COLORS[index % PLATFORM_CHART_COLORS.length]} />)}
                </Pie>
                <Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={formatTooltipValue("Engagement")} />
                <Legend verticalAlign="bottom" height={28} iconType="circle" />
              </PieChart>
            </ResponsiveContainer>
          ) : <p className="flex h-full items-center justify-center text-sm text-muted">No platform engagement data yet.</p>}
        </ChartCard>
