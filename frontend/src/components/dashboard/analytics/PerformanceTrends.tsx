  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <AreaChart data={data}>
        ...
        <Area type="monotone" dataKey={dataKey} stroke={color} fill={`url(#${gradientId})`} strokeWidth={3} dot={data.length <= 20} />
      </AreaChart>
    </ResponsiveContainer>
  );
}
...
export default function PerformanceTrends() {
  const { postAnalytics, trendPoints, audienceSnapshots, isLoading, hasLoaded, fetchAnalytics } = useAnalyticsStore();
  ...
  const filteredTrend = useMemo(() => {
    ...
    postAnalytics.forEach((row) => {
      const post = posts.find((item) => String(item.id) === row.postId);
      if (platform !== "all" && row.platform !== platform) return;
      if (campaignId !== "all" && String(post?.campaignId ?? "") !== campaignId) return;
      if (contentType !== "all" && post?.contentType !== contentType) return;
      if (!isWithinRange(row.date, dateRange)) return;
    ...
    if (byDate.size > 0) return Array.from(byDate.values()).sort((a, b) => a.date.localeCompare(b.date));
    return trendPoints.filter((point) => isWithinRange(point.date, dateRange));
  }, [postAnalytics, posts, trendPoints, platform, campaignId, contentType, dateRange]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <ChartCard title="Engagement Trend">
          <TrendLineChart data={bucketedRows} dataKey="engagement" color="#d99b00" tooltipLabel="Engagement" />
        </ChartCard>

        <ChartCard title="Reach Trend">
          <TrendLineChart data={bucketedRows} dataKey="reach" color="#3e6da3" tooltipLabel="Reach" />
        </ChartCard>
        ...
        <ChartCard title="Clicks Trend">
          <TrendLineChart data={bucketedRows} dataKey="clicks" color="#a8452f" tooltipLabel="Clicks" />
        </ChartCard>
