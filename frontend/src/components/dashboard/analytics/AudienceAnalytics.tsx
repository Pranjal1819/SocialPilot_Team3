...
export default function AudienceAnalytics() {
  ...
  }, [fetchAnalytics, fetchCampaigns, fetchPosts, businessOwnerId, pathname]);
  ...
  // No historical follower series exists in the API response, so create a
  // visible projection from a one-follower baseline to the current total.
  // Easing and small deterministic variation keep the line readable.
  const followersGrowthSeries = useMemo(() => {
    ...
    return Array.from({ length: days }, (_, i) => {
      ...
      const easedProgress = Math.pow(progress, 1.55);
      const variation = i === 0 || i === days - 1 ? 0 : ((i % 5) - 2) * Math.max(1, totals.followers * 0.006);
      const value = i === 0
        ? 1
        : i === days - 1
          ? totals.followers
          : Math.max(1, Math.round(1 + (totals.followers - 1) * easedProgress + variation));
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">Audience demographics, locations, and activity patterns.</p>
      </div>
      ...
      <ChartCard title="Followers Growth Graph">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={followersGrowthSeries}>
            ...
            />
            <Line type="monotone" dataKey="followers" stroke="#d99b00" strokeWidth={3} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </ChartCard>
