...
export default function ContentAnalytics() {
  ...
  useEffect(() => {
    fetchCampaigns(businessOwnerId);
  ...
  const rows: ContentRow[] = useMemo(() => {
    ...
    return postAnalytics
      .map((analytics) => {
        ...
        if (!post) return null;
        const campaign = post.campaignId ? getCampaignById(String(post.campaignId)) : undefined;
              ...
              <tbody>
                {compareRows.map((row) => (
                  <tr key={rowKey(row)} className="border-b border-border last:border-b-0">
                    ...
                    <td className="py-2 font-medium">{row.analytics.engagementRate.toFixed(1)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {rows.length === 0 ? (
          ...
          <div className="divide-y divide-border">
            {rows.map((row) => {
              ...
              return (
                <div key={key} className="flex min-w-0 flex-col gap-3 p-4 sm:flex-row sm:items-center">
                  <label className="flex min-w-0 items-start gap-3 sm:flex-1">
                    <input
                      type="checkbox"
                      checked={isComparing}
                    ...
                    <div className="min-w-0">
                      <p className="break-words text-sm font-medium text-ink">{row.post.caption}</p>
                  ...
                  <div className="grid grid-cols-4 gap-x-4 gap-y-1 text-xs sm:grid-cols-8 sm:text-right">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-ink">{row.analytics.engagementRate.toFixed(1)}%</p>
                    </div>
                  </div>
                </div>
              );
