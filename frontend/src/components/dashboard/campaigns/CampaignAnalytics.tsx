...
export default function CampaignAnalytics({ campaignId }: { campaignId: string }) {
  ...
  const postAnalytics = useAnalyticsStore((state) => state.postAnalytics);
  ...
  useEffect(() => {
    fetchCampaigns();
    fetchPosts().then((loadedPosts) => fetchAnalytics(loadedPosts.length, !pathname.startsWith("/content-creator")));
  }, [fetchCampaigns, fetchPosts, fetchAnalytics, pathname]);
  ...
  const campaignPosts = posts.filter((post) => String(post.campaignId ?? "") === String(campaign.id));
  const campaignAnalytics = postAnalytics.filter((row) => campaignPosts.some((post) => String(post.id) === row.postId));
  const publishedPosts = campaignPosts.filter((post) => post.status === "published").length;
  const scheduledPosts = campaignPosts.filter((post) => post.status === "scheduled").length;
  const engagement = campaignAnalytics.reduce((sum, row) => sum + row.likes + row.comments + row.shares, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Scheduled Posts" value={scheduledPosts || campaign.scheduledPosts} />
        <StatCard label="Published Posts" value={publishedPosts || campaign.publishedPosts} />
      ...
      {metrics ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <StatCard label="Engagement" value={formatCount(engagement || metrics.engagement)} />
