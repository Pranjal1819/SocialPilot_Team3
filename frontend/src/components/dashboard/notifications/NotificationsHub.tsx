  ...
  return (
    <div>
      <div className="flex flex-wrap gap-1 border-b border-border">
        {tabs.map((tab) => (
          <button key={tab.key} onClick={() => setActive(tab.key)} className="relative flex items-center gap-2 px-4 py-3 text-sm">
            ...
            {tab.key === "center" && unreadCount > 0 ? <span className="flex h-4 min-w-4 items-center justify-center bg-accent px-1 font-mono text-[10px] text-ink">{unreadCount}</span> : null}
            {active === tab.key ? <motion.span layoutId="notif-hub-tab-underline" className="absolute inset-x-0 -bottom-px h-[2px] bg-accent" /> : null}
          </button>
        ))}
      </div>
      <div className="mt-6">
