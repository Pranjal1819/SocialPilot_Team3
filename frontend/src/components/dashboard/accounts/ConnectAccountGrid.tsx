...
export default function ConnectAccountGrid() {
  ...
  // Handle incoming OAuth success/failure query parameters.
  useEffect(() => {
  ...
  // Content creators can connect the platforms supported by their workflow.
  const visiblePlatforms = SOCIAL_PLATFORMS.filter(
    (platform) => ["instagram", "linkedin", "youtube"].includes(platform)
  );
